...
<template>
    <div class="flex flex-row place-items-center p-4" @click="viewOrganization">
      <img :src="organization.encodedImage" alt="🏢" class="w-16 h-16 rounded-full drop-shadow-xl mr-4 text-center text-4xl" />
      <div class="flex-1 flex flex-col">
        <div class="text-lg font-bold text-gray-800 text-left">{{ organization.orgName }}</div>
        <div class="text-sm text-gray-500 text-left">Created by user: {{ organization.orgOwnerID }}</div>
      </div>
    </div>
  </template>
