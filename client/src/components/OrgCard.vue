<template>
  ...
  >
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="mt-1 space-y-1">
          <div class="flex items-center text-sm text-gray-500">
            ...
            <span class="truncate">Created by: {{ organization.orgOwnerID }}</span>
          </div>
          ...
          <div v-if="organization.description" class="flex items-center text-sm text-gray-500">
            <svg class="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span class="truncate">{{ organization.description || 'No description available' }}</span>
          </div>
        </div>
      </div>
    ...
    <!-- Hover Actions -->
    <div class="absolute inset-x-0 bottom-0 h-0.5 bg-secondary transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-200"></div>
  </div>
</template>
