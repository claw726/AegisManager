<template>
  ...
  >
        ...
        <div class="flex flex-col md:flex-row gap-8">
          ...
          <div class="flex flex-col items-center">
            <div class="relative group">
              <img
                :src="proj.encodedImage || 'https://d31kswug2i6wp2.cloudfront.net/fallback/company/medium_logo_default.png'"
          ...
          <div class="flex-1 space-y-4">
            <h1 class="text-4xl font-bold text-gray-900">{{ proj.projectName }}</h1>
            <p class="text-xl text-gray-600">{{ proj.projectDescription }}</p>
            <div class="flex items-center space-x-2 text-sm text-gray-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                <path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
              </svg>
              <span v-if="creator.userName">Created by {{ creator.userName }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <button @click="goToCreateTask" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition duration-200">
    ...
    <!-- List of Tasks -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div v-if="tasks && tasks.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <TaskCard
