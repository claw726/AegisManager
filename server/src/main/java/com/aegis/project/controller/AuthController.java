...
public class AuthController {
  ...
  public ResponseEntity<String> createUser(
    ...
    logger.info(
      "Received registration request with email: {}, name: {}, password: {}, profilePicture: {}",
      email,
      name,
      password,
      profilePicture
  ...
  public ResponseEntity<String> requestPasswordReset(
    ...
    try {
      ...
      if (existingToken != null) {
        logger.info(
          "Deleting existing password reset token for user: {}",
          email
        );
      ...
      return ResponseEntity.ok(
        "Password reset requested successfully with token: " + token
      );
      ...
      return ResponseEntity.internalServerError()
        .body("Error requesting password reset");
    }
  }
  ...
  public ResponseEntity<String> enable2FA(@RequestParam int userID) {
    try {
      ...
      String qrCodeURL = twoFactorAuthService.getQRBarcodeURL(
        userID,
        secretKey
      );
      ...
      if (e.getMessage().equals("User not found with ID: " + userID)) {
        ...
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(
          "User does not have permission to enable 2FA for user with ID: " +
          userID
        );
      } else {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
    ...
    try {
      ...
      if (secretKey == null) {
        return ResponseEntity.badRequest()
          .body("2FA not enabled for user with ID: " + userID);
      }
  ...
  public ResponseEntity<String> disable2FA(@RequestParam int userID) {
    try {
      ...
      if (e.getMessage().equals("User not found with ID: " + userID)) {
        ...
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(
          "User does not have permission to disable 2FA for user with ID: " +
          userID
        );
      } else {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
