...
public class ProjectServiceTest {
  ...
  public void testCreateProject_Success() {
    ...
    when(
      projectRepository.existsProjectByOrgAndName(parentOrgID, projectName)
    ).thenReturn(false);
    when(userRepository.findById(projectOwnerID)).thenReturn(
      Optional.of(owner)
    );
    ...
    boolean isCreated = projectServiceSpy.createProject(
      projectName,
      projectDescription,
      projectOwnerID,
      parentOrgID,
    ...
    ArgumentCaptor<ProjectModel> projectCaptor = ArgumentCaptor.forClass(
      ProjectModel.class
    );
    ...
    assertEquals(
      "Description",
      projectCaptor.getValue().getProjectDescription()
    );
  }
  ...
  public void testCreateProjectJson_Success() {
    ...
    when(userService.createUserJson(user1)).thenReturn(
    ...
    assertTrue(
      projectJson.contains(
        "\"assignedUsers\": [{\"userID\": 1,\"userName\": \"User1\",\"email\": \"user1@example.com\"}]"
      )
    );
  }
  ...
  public void testGetAllTasksFromProject_Success() {
    ...
    when(taskRepository.findByParentProjectID(projectID)).thenReturn(
      List.of(task1, task2)
    );
  ...
  public void testGetAllTasksForUser_Success() {
    ...
    when(
      taskRepository.findByParentProjectIDAndUserID(
        projectID,
  ...
  public void testGetAllUserProjects_Success() {
    ...
    when(projectRepository.findAllUserProjects(user.getUserID())).thenReturn(
