...
public class TaskServiceTest {
    ...
    public void testSwitchTaskAssigner_Success() {
        ...
        when(userRepository.findByEmail(newAssignerEmail)).thenReturn(Optional.of(newAssigner));
    ...
    public void testDirectlySwitchTaskAssigner_Success() {
        ...
        String result = taskService.directlySwitchTaskAssigner(taskID, newAssignerEmail);
    ...
    public void testAssignerDecision_Success() {
        ...
        assertEquals("Assigner successfully switched for task with ID: " + taskID, result);
    }
    ...
    public void testSendTaskInfoToUsers_Success() {
        ...
        verify(simpMessageTemplate, times(1)).convertAndSendToUser(anyString(), eq("/queue/task-updates"), any());
    }
    ...
    public void testNotifyTaskDeletion_Success() {
        ...
        verify(simpMessageTemplate, times(1)).convertAndSendToUser(eq("user@example.com"), eq("/queue/task-updates"), any());
    }
    ...
    public void testCreateTask_Success() {
        ...
        when(taskRepository.existsTaskByProjectAndName(parentProjectID, taskName)).thenReturn(false);
        when(projectRepository.findById(parentProjectID)).thenReturn(Optional.of(parentProject));
        when(orgRepository.findById(parentOrgID)).thenReturn(Optional.of(new OrgModel()));
        when(userRepository.findByEmail("currentUser@example.com")).thenReturn(Optional.of(currentUser));
        ...
        boolean result = taskServiceSpy.createTask(parentProjectID, parentOrgID, taskName, taskDescription, assignerID, taskPriority, dueDate);
    ...
    public void testCreateTask_TaskAlreadyExists() {
        ...
        when(taskRepository.existsTaskByProjectAndName(parentProjectID, taskName)).thenReturn(true);

        Exception exception = assertThrows(RuntimeException.class, () -> {
            taskService.createTask(parentProjectID, 1, taskName, "Description", 1, "High", new Date());
        });

        assertEquals("Task with given name already exists in project", exception.getMessage());
    }
    ...
    public void testUpdateTask_TaskNotFound() {
        ...
        Exception exception = assertThrows(RuntimeException.class, () -> {
            taskService.updateTask(taskID, "Updated Task", "Updated Description", 1, "Medium", new Date(), true);
        });
    ...
    public void testDeleteTask_Success() {
        ...
        when(taskRepository.findById(taskID)).thenReturn(Optional.of(task));
        when(userRepository.findByEmail("currentUser@example.com")).thenReturn(Optional.of(currentUser));
        when(projectRepository.findById(task.getParentOrgID())).thenReturn(Optional.of(new ProjectModel()));
    ...
    public void testUpdateTaskCompletionStatus_Success() {
        ...
        verify(taskRepository, times(1)).updateTaskCompletedStatus(taskID, isCompleted);
    }
