...
public class OrgServiceTest {
    ...
    public void testCreateOrg_Success() {
        ...
        boolean isCreated = orgServiceSpy.createOrg("New Org", "Description", 1, "image");

        assertTrue(isCreated);

        ArgumentCaptor<OrgModel> orgCaptor = ArgumentCaptor.forClass(OrgModel.class);
    ...
    public void testGetAllOrgs_Success() {
        ...
        List<OrgDTO> orgs = orgService.getAllOrgs().stream()
    ...
    public void testGetAllProjectsFromOrg_Success() {
        ...
        when(orgRepository.findById(orgID)).thenReturn(Optional.of(org));
        when(projectRepository.findByParentOrgID(orgID)).thenReturn(List.of(project1, project2));
        when(authentication.getPrincipal()).thenReturn(userDetails);
        when(userDetails.getUsername()).thenReturn("currentuser@example.com");
        when(userRepository.findByEmail("currentuser@example.com")).thenReturn(Optional.of(currentUser));
    ...
    public void testUpdateOrg_Success() {
        ...
        orgService.updateOrg(orgID, "Updated Org", "Updated Description", 1, "image");
    ...
    public void testDeleteOrg_NoPermission() {
        ...
        assertEquals("User does not have permission to delete org", exception.getMessage());
    }
    ...
    public void testAddUser_NoPermission() {
        ...
        assertEquals("User does not have permission to add user to org", exception.getMessage());
    }
    ...
    public void testRemoveUser_NoPermission() {
        ...
        assertEquals("User does not have permission to remove user from org", exception.getMessage());
    }
    ...
    public void testGetArchivedProjects_Success() {
        ...
        when(userDetails.getUsername()).thenReturn("currentuser@example.com");
        when(userRepository.findByEmail("currentuser@example.com")).thenReturn(Optional.of(currentUser));
        when(projectRepository.findByParentOrgID(orgID)).thenReturn(List.of(project));
    ...
    public void testGetArchivedProjects_NoPermission() {
        ...
        assertEquals("There are no archived projects accessible by this user", exception.getMessage());
    }
    ...
    public void testCreateOrgJson() {
        ...
        when(userService.createUserJson(user)).thenReturn("{\"userID\": 1,\"userName\": \"Test User\",\"email\": \"test@example.com\"}");

        String orgJson = orgService.createOrgJson(org);

        assertEquals("{\"orgID\": 1,\"orgName\": \"Test Org\",\"orgDescription\": \"Description\",\"orgOwnerID\": 1,\"encodedImage\": \"image\",\"users\": [{\"userID\": 1,\"userName\": \"Test User\",\"email\": \"test@example.com\"}]}", orgJson);
    }
}